...
class PagesController extends AppController
{
    ...
    {
        // pr($siteoptions['web_layout']);die();
        parent::initialize();
        $siteoptions = $this->siteoptions();
        // pr($siteoptions['web_layout']);die();
        $this->viewBuilder()->setLayout('newsbd');
...
# ============================================ 
    public function videos($video_id = null){
        ...
        if(!empty($video_id)){
            $video = $this->Upvideos->find('all',['conditions'=>['Upvideos.id'=>$video_id]])->first();
            $this->set(compact('video'));
        }
        //latest products
        //$videos = Cache::read('videos');
        if(empty($videos)){
            $videos = json_encode($this->Upvideos->getVides(12));
            //pr($videos);die();
...
# ============================================ 
    public function index(){
        ...
        if(!empty($siteoptions['web_lead_count'])){
            $lead_count = $siteoptions['web_lead_count'];
        }else{
            $lead_count = 5;
        }
        
        $products_lead = $this->Products->getLiveNews(1, $lead_count);
        ...
        if(empty($products_special)){
            ...
            $this->Tags->find()
             ...
             ->contain([
                'Products' => function($q) {
                    return $q
                        ->where(['Products.status'=>1,'Products.genus'=>1,'Products.ishome'=>1])
...
# ============================================ 
    public function topics($slug = null,$page = null){
        if(!empty($slug)){
            ...
            $cat_details = json_encode($table_tags->find('all', ['conditions'=>['Tags.slug'=>$slug],'contain'=>['ChildTags']])->first());
	        //pr($products_latest);die();
