                        ...
                        foreach($data['tags']['_ids'] as $tag){
                            ...
                            $products_tagged = json_encode($table_tags->find()
                                ...
                                ->contain([
                                    'Products' => function($q) {
                                        return $q
                                            ->where(['status'=>1,'genus'=>1,'ishome'=>1])
                                            ->select(['Products.id','Products.title','Products.created','Products.ishome'])
                                            ->order(['id' =>'ASC'])
                                            ->limit(5);
                                    }
                                ])
                            );
                            $setData = json_decode($products_tagged,true);
                            // pr($setData);die();
                            $setDataSet['id'] = $setData[0]['products'][0]['id'];
                            $setDataSet['ishome'] = 0;
                            // pr($setDataSet);die();
                            $productSet = $this->Products->get($setData[0]['products'][0]['id']);
                            $ishome = $this->Products->patchEntity($productSet, $setDataSet);
                            $resultSet = $this->Products->save($ishome);
                        }
                    }
                    // end rundown remover
