...
class PagesController extends AppController
{
    public function initialize(): void
    {
        ...
        if(empty($products_latest)){
            $products_latest = json_encode($this->Products->users_products(10));
        ...
        $this->Auth->allow(['index','topics','view','cats','news','details','archive','photos','videos','search','page']);
    }
...
# ============================================ 
    public function index(){
        ...
        $videos = file_get_contents("https://youtube.googleapis.com/youtube/v3/search?part=snippet&channelId=UCY3oxPh3cvbOJ376ci7RIjQ&maxResults=5&order=date&key=AIzaSyCXjhwDt6u01B1Tk2Kr8upXTF5alK3cKEg");
        //pr($video);die();
        //latest products
        $products_latest = Cache::read('products_latest','oneweek');
        if(empty($products_latest)){
        ...
        if(empty($products_top)){
            $products_top = json_encode($this->Products->users_product_top(10));
            Cache::write('products_top', $products_top, 'oneweek');
        }

        //Today's Feature = Box Heading
        if(empty($products_headingbox)){
            $products_headingbox = json_encode($this->Products->users_product_headingbox(15));
            Cache::write('products_headingbox', $products_headingbox, 'oneweek');
        }

        //Today's Feature = Box Heading
        if(empty($products_featurebox)){
            $products_featurebox = json_encode($this->Products->users_product_featurebox(15));
            Cache::write('products_featurebox', $products_featurebox, 'oneweek');
        }
        ...
        $this->set(compact('products_top','products_special','videos','settings','products_latest','products_headingbox','products_featurebox','exclusive'));
    }
                 ...
                 ->contain([
                    'Products' => function($q) {
                        return $q
                            ...
                            ->select(['Products.id','Products.title','Products.slug','Products.created','Products.details'])
                            ->contain(['Uploads' => function($p) {
                                            return $p
                                                ->where(['status'=>1])
                                                ->select(['Uploads.id','Uploads.imgname','Uploads.caption','Uploads.product_id'])
...
# ============================================
    ...
    public function view($slug = null)
    {
        ...
        $table_tags = TableRegistry::getTableLocator()->get('Tags');
        $products_tagged = json_encode($table_tags->find()
        ->where(['id'=>$product_details['tags'][0]['id']])
        ->select(['id','title','slug'])
        ->contain([
           'Products' => function($q) {
               return $q
        ...
        $this->set(compact('product','products_latest','products_top','comment','products_tagged'));
    }
